...
fn parse(input_path: &PathBuf) -> Result<(u64, u64, u64), Box<dyn std::error::Error>> {
...
pub fn part1(input_path: &PathBuf) -> Result<u64, Box<dyn std::error::Error>> {
    let (boss_hit_points, boss_damage, boss_armor) = parse(input_path)?;
            ...
            for ring1 in RINGS {
                for ring2 in RINGS {
                    if ring1 == ring2 && ring2 != RINGS[0] {
                        continue; // the same ring can only be equipiped once, except for no ring. 
                    }
...
pub fn part2(input_path: &PathBuf) -> Result<u64, Box<dyn std::error::Error>> {
        ...
        for armor in ARMOR {
            for ring1 in RINGS {
                for ring2 in RINGS {
                    ...
                    let player_damage: u64 = [weapon.1, armor.1, ring1.1, ring2.1].iter().sum();
                    let player_armor: u64 = [weapon.2, armor.2, ring1.2, ring2.2].iter().sum();
                    let effective_player_damage =
                        player_damage.checked_sub(boss_armor).unwrap_or(1).max(1);
                    let effective_boss_damage =
                        boss_damage.checked_sub(player_armor).unwrap_or(1).max(1);

                    let player_rounds = 100_u64.div_ceil(effective_boss_damage);
                    let boss_rounds = boss_hit_points.div_ceil(effective_player_damage);

                    if player_rounds < boss_rounds {
                        if let Some(ref mut max_cost) = max_cost {
                            if *max_cost < player_cost {
                                *max_cost = player_cost;
                            }
                        } else {
                            max_cost = Some(player_cost)
                        }
                    }
                }
            }
    ...
    max_cost.ok_or("no losing combination found".into())
}
